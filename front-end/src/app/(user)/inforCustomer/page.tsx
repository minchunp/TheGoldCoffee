...
const InforCustomer = () => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (token) {
          ...
          //
          const UoderResponse = orderResponse.data.filter(
            (Uorder: Oder) => Uorder.id_user === userId
          );
  ...
  useEffect(() => {
    const calculateOrderCounts = () => {
      const counts = {
        order: showOder.filter((order) => order.status === "chờ xác nhận")
          .length,
        confirm: showOder.filter((order) => order.status === "đã xác nhận")
          .length,
        transit: showOder.filter((order) => order.status === "đang giao hàng")
          .length,
        complete: showOder.filter((order) => order.status === "đã giao hàng")
          .length,
        rejected: showOder.filter((order) => order.status === "hủy đơn hàng")
          .length,
      };
  ...
  const cancelOrder = async (id: string) => {
    try {
      ...
      setFilterOder(
        updatedOrders.filter((order) => order.status === oderStatus)
      );
    } catch (error) {
  ...
  return (
    <>
      <ModalOrderDetail
        id={selectedOrderDetailId}
        isOpen={isModalOpenOrderDetail}
        onClose={closeModal}
      />
        ...
        <div className="boxcenter">
          ...
          <div className="order-information">
            <div className="main-icon-order-information">
              ...
              >
                <div className="quantity-order-information">
                  {orderCount.order}
                </div>
            ...
            <div className="body-order-information">
              ...
              <div className="content-body-order-information">
                ...
                {filterOder.length > 0 ? (
                    ...
                    <div key={order.id} className="item-order-information">
                      ...
                      <div className="container-btn-func-order-information">
                        <p
                          ...
                          className="btn-check-orderDetail"
                        >
                          Xem chi tiết
                        </p>
                        {order.status === "chờ xác nhận" && (
                          <p
                            ...
                            className="btn-check-orderDetail btn-check-orderDetail__cancle"
                          >
                            Huỷ đơn hàng
                          </p>
                        )}
                  ...
                  <div className="empty-item-order-information">
                    Không có đơn hàng!
                  </div>
                )}
              </div>
